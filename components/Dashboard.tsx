...
const Dashboard: React.FC<DashboardProps> = ({ onModuleSelect, theme, onToggleTheme }) => {
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-white p-4 md:p-8 pb-24">
        ...
        <header className="flex flex-col-reverse gap-4 md:flex-row justify-between items-center mb-10 max-w-6xl mx-auto">
            <div className="flex items-center gap-4 w-full md:w-auto glass-panel px-6 py-3 rounded-3xl animate-pop">
                ...
                <div>
                    <h1 className="text-xl md:text-2xl font-bold truncate">مرحباً، {user?.name}</h1>
                    <p className="text-gray-500 dark:text-gray-400 text-sm">واصل رحلة التعلم الخاصة بك</p>
                </div>
            </div>
            ...
            <div className="flex items-center gap-3 w-full md:w-auto justify-end">
                ...
                <button 
                    onClick={logout}
                    className="btn-liquid-secondary px-6 py-3 rounded-2xl text-sm font-bold whitespace-nowrap flex items-center gap-2 group"
                >
        ...
        <div className="max-w-6xl mx-auto glass-panel bg-white/80 dark:bg-gray-800/80 rounded-3xl p-8 shadow-xl mb-12 border border-white/50 dark:border-white/10 relative overflow-hidden animate-pop" style={{animationDelay: '0.1s'}}>
            <div className="absolute top-0 left-0 w-full h-1 bg-gray-200 dark:bg-gray-700">
                <div 
                    className="h-full bg-gradient-to-r from-blue-500 to-purple-500 transition-all duration-1000 shadow-[0_0_10px_rgba(59,130,246,0.5)]"
                    style={{ width: `${percentage}%` }}
                ></div>
            </div>
            <div className="flex flex-col md:flex-row justify-between items-center gap-6">
                <div>
                    <h2 className="text-2xl font-bold mb-2 bg-clip-text text-transparent bg-gradient-to-r from-gray-900 to-gray-600 dark:from-white dark:to-gray-300">تقدم الدورة التدريبية</h2>
                    <p className="text-gray-500 dark:text-gray-400 text-lg">لقد أكملت {completed} من أصل {totalModules} وحدات تعليمية.</p>
                </div>
                    ...
                    <div className="relative w-24 h-24">
                        <svg className="w-full h-full transform -rotate-90" viewBox="0 0 36 36">
                            ...
                            <path
                                ...
                                className="animate-[dash_1s_ease-out] drop-shadow-md"
                                strokeLinecap="round"
                            />
                            <defs>
            ...
            {modules.map((module, index) => {
                ...
                return (
                    <div key={module.id} className="relative z-10 animate-pop" style={{animationDelay: `${0.2 + (index * 0.1)}s`}}>
                        <div 
                            className={`group flex flex-col md:flex-row items-center gap-6 md:gap-12 transition-all duration-500
                                ${isLocked ? 'opacity-60 grayscale' : 'opacity-100'}
                            `}
                        >
                            {/* Icon/Marker */}
                            <div className={`
                                w-16 h-16 rounded-full flex-shrink-0 flex items-center justify-center border-4 shadow-xl transition-all duration-300 z-20
                                ${isCompleted ? 'bg-green-500 border-green-200 text-white scale-110' : 
                                  isLocked ? 'bg-gray-200 dark:bg-gray-700 border-gray-300 dark:border-gray-600 text-gray-400' :
                                  'bg-white dark:bg-gray-800 border-blue-500 text-blue-600 animate-pulse-slow'}
                            `}>
                                {isCompleted ? (
                            ...
                            <button
                                ...
                                disabled={isLocked}
                                className={`flex-1 w-full text-right glass-panel p-6 rounded-3xl transition-all duration-300 relative overflow-hidden text-left
                                    ${isLocked 
                                        ? 'cursor-not-allowed bg-gray-50/50 dark:bg-gray-900/50' 
                                        : 'bg-white/90 dark:bg-gray-800/90 hover:shadow-[0_20px_50px_-12px_rgba(0,0,0,0.2)] transform hover:-translate-y-2 cursor-pointer'}
                                `}
                            >
                                <div className="flex justify-between items-start mb-2">
                                    ...
                                    <span className={`text-xs font-bold px-3 py-1 rounded-full uppercase tracking-wide
                                        ${isCompleted ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400' : 
                                          isLocked ? 'bg-gray-100 text-gray-500 dark:bg-gray-700 dark:text-gray-400' :
                                          'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400'}
                                    `}>
                                        {isCompleted ? 'مكتمل' : isLocked ? 'مغلق' : 'متاح'}
                                    </span>
                                </div>
                                ...
                                <div className="flex items-center justify-between mt-6">
                                    {isCompleted && module.quiz ? (
                                        <span className="text-sm font-medium text-gray-500 dark:text-gray-400 flex items-center gap-1">
                                            <span>⭐</span> نتيجة الاختبار: {progress.quizScores[module.id] || 0}%
                                        </span>
                                    ) : <span></span>}
